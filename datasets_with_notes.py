# code is copied from https://github.com/fastai/fastai/blob/master/fastai/datasets.py#L221
# added notes. Search "Note" to see them
...
# Note: return the string after the last /, i.e. a file name after removing the path. for example, url2name("A/B/C/D") returns D
def url2name(url): return url.split('/')[-1]

#TODO: simplify this mess

# Note: get the name from the url (removing the path), then return data path. Say the url is 
# 'http://files.fast.ai/data/examples/imdb_sample', we return current_path/data/imdb_sample or 
# ~/.fastai/data/imdb_sample
...
'''
Note: what happends when we call this line of code:
path = untar_data(URLs.IMDB_SAMPLE)
URLs.IMDB_SAMPLE has this value: 'http://files.fast.ai/data/examples/imdb_sample'
it'll download imdb_sample.tgz to ~/.fastai/data
then unzip the file to ~/.fastai/data/imdb_sample

test:
rm -r ~/.fastai
Then execute this line from python prompt:
path=untar_data(URLs.IMDB_SAMPLE)
On Mac, it creates .fastai directory, under it, downloads config.yml which contains
  1 data_path: ~/.fastai/data
  2 model_path: ~/.fastai/models
creates data directory, under it, 
downloads imdb_sample.tgz
creates imdb_sample directory

details:
step 0: since URLs.IMDB_SAMPLE has this value: http://files.fast.ai/data/examples/imdb_sample
and dest is None, we call url2path(url, data) with data has the default value of True
which calls datapath4file("imbd_sample", ".tgz", archive=False)
in datapath4file, current_path/data/file_name or current_path/data/file_name.tgz exists, return the entire path
else if archive is True, return archive path,
else return ~/.fastai/data/filename
the above returned value is assigned to variable "dest"

step 1: fname = Path(ifnone(fname, _url2tgz(url, data)))
ifnone is defined in fast.ai
def ifnone(a:Any,b:Any)->Any:
    "`a` if `a` is not None, otherwise `b`."
    return b if a is None else a

since initially fname is None, we call _url2tgz(url, True)
which returns current_path/data/imdb_sample

step 2, force_download is set to False, size and hash of the file might not be correct
        remove current file and directory
        download_data
        unzip it 
'''
...
#Note: if data is True (the default value), return current_path/data/file_name
# else, return current_path/models/file_name
# for example, f'{url2name(url)}{ext}' is 'imdb_sample.tgz'
...
# Note: if current_path/models/file_name or current_path/models/file_name.tgz exists, 
# return current_path/models/file_name
# else, return ~/.fastai/models/filename
...
# Note: if current_path/data/file_name or current_path/data/file_name.tgz exists, 
...
# Note: the initial null destination will be set to current_path/data/filename_from_url or ~/.fastai/data/filename_from_url
